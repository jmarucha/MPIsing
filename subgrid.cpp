...
subgrid::subgrid(int w, int h, double j, double field)
  ...
  MPI_Send_init(grid + w * (h - 1), w, MPI_INT, next, 0, MPI_COMM_WORLD,
                requests.bot_send());

  MPI_Recv_init(ghost_top, w, MPI_INT, prev, 0, MPI_COMM_WORLD,
                requests.top_recv());
  MPI_Recv_init(ghost_bottom, w, MPI_INT, next, 0, MPI_COMM_WORLD,
                requests.bot_recv());
...
int &subgrid::point(int x, int y) {
  if (x == -1)
    return grid[w * y + w - 1]; // left PBC;
  if (x == w)
    return grid[w * y]; // right PBC;
  if (psize > 1) {      // multiprocessing
    if (y == -1)
      return ghost_top[x]; // ghost_line;
    if (y == h)
      return ghost_bottom[x]; // ghost_line;
  } else {
    if (y == -1)
      return grid[w * (h - 1) + x]; // top PBC;
    if (y == h)
      return grid[x]; // bottom PBC;
  }
  return grid[w * y + x];
}
